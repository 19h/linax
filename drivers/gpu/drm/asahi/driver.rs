...
use kernel::dma::dma_set_mask_and_coherent;
...
impl platform::Driver for AsahiDriver {
    ...
    fn probe(pdev: &mut platform::Device, info: Option<&Self::IdInfo>) -> Result<Pin<KBox<Self>>> {
        ...
        to_result(dma_set_mask_and_coherent(
            pdev.as_ref(),
            (1 << cfg.uat_oas) - 1,
        ))?;
        ...
        let data_init = |drm: ARef<drm::device::Device<AsahiDriver>>| -> Result<Arc<AsahiData>> {
        ...
        let drm = drm::device::Device::<AsahiDriver>::new_from_closure(dev.as_ref(), data_init)?;
